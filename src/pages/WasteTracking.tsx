...
const WasteTracking = () => {
  ...
  const fetchWasteEntries = async () => {
    ...
    setIsLoading(true);
    try {
      // Define the proper types for waste entries in Supabase
      interface SupabaseWasteEntry {
        id: string;
        user_id: string;
        date: string;
        waste_type: string;
      ...
      // Map the Supabase data to our WasteEntry interface
      if (data) {
        const mappedEntries: WasteEntry[] = data.map((entry: SupabaseWasteEntry) => ({
          id: entry.id,
          date: entry.date,
  ...
  const addWasteEntry = async () => {
    ...
    try {
      ...
      if (data && data[0]) {
        ...
        const newEntryWithId: WasteEntry = {
          id: data[0].id,
          date: data[0].date,
          type: data[0].waste_type,
          weight: data[0].weight,
          recyclable: data[0].recyclable
        };
            ...
            <Card className="lg:col-span-1 shadow-sm">
              ...
              <CardContent className="p-4 sm:p-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <Label htmlFor="type">{t('wasteType')}</Label>
                    <Select 
                      onValueChange={(value) => handleSelectChange('type', value)}
                      value={newEntry.type || undefined}
                    >
                      ...
                      <tbody>
                        {entries.map((entry) => (
                          <tr key={entry.id} className="border-b hover:bg-gray-50">
                            ...
                            <td className="py-3 px-4">{getWasteTypeTranslation(entry.type)}</td>
